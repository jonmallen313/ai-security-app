...
const ChatDialog: React.FC<ChatDialogProps> = ({ incident, onSendMessage, isLoading, onClose, messages, setMessages }) => {
  ...
  return (
    <div
      className={`fixed bottom-4 right-4 z-50 transition-all duration-300 ${isExpanded ? 'w-96 h-96' : 'w-32 h-12'
        }`}
    >
      <div className="bg-[#1e1e1e] text-white rounded-md border shadow-md opacity-90 overflow-hidden flex flex-col">
        <div className="bg-[#333] p-2 cursor-move flex items-center justify-between" onClick={toggleOpen}>
